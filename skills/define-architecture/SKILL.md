...
6. **Dependencies** — note which architectural dependencies will become `blocked_by` links in downstream stories. On reconcile, also hand off any migration story (from the classification table) for `define-backlog` to intake, flagged `breaking: true` so it lands on whichever epic carries it — that flag is what lets `scripts/bump-version.mjs`'s `epic-driven` mode suggest a major version bump instead of re-deriving it from ADRs by hand.
