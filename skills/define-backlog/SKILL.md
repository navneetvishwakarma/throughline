...
## Do this
1. Cluster related REQ-xx into vertically-sliced epics. Seed a Foundation enabler epic (`order: 0`, `vertical: false`) first. Tech/arch debt promoted from `docs/engineering/tech-debt.md` (a `REQ-xx` `define-product` added on its behalf) clusters the same way — into an existing or new `vertical: false` epic, never a vertical one.
